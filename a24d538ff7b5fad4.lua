...
local function tweenBoatToVector3(targetPos)
    ...
    local goal = {
        CFrame = CFrame.new(targetPos.X, finalY, targetPos.Z)
    }
...
local function tweenBoatToPlayer(targetPlayer)
    ...
    local tweenInfo = TweenInfo.new(tweenTime, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
    local goal = {
        CFrame = CFrame.new(playerPos.X, finalY, playerPos.Z)
    }
        ...
        if lastBoat and lastSeat and lastBoat.Parent then
            ...
            if hrp then
                ...
                if distance > 10 then
                    ...
                    local goal = {
                        CFrame = CFrame.new(seatPos + Vector3.new(0, 3, 0))
                    }
...
local function killAuraLoop()
    ...
    while killAuraEnabled do
        ...
        if not hrp or not monstersFolder then
            continue
        end
...
local function createESP(fruit)
    if not fruit:FindFirstChild("Handle") then
        return
    end
...
local function tweenBoatToProtectHeight(boat)
    ...
    local tweenInfo = TweenInfo.new(tweenTime, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
    local goal = {
        CFrame = CFrame.new(currentPos.X, targetY, currentPos.Z)
    }
