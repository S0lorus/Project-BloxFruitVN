...
local Window = Luna:CreateWindow({
    Name = "RelaxHub",
    Subtitle = nil,
    LogoID = "rbxassetid://7229442422",
    LoadingEnabled = true,
    LoadingTitle = "<Đang khởi chạy Hub đợi chút>",
    LoadingSubtitle = "Dev bởi @thanhtaihy11 (AKA Relax)",

    ConfigSettings = {
		RootFolder = nil, 
		ConfigFolder = "RelaxHub" 
    },
})

Window:CreateHomeTab({
	SupportedExecutors = {}, -- A Table Of Executors Your Script Supports. Add strings of the executor names for each executor.
	DiscordInvite = "thanhtaihy11",
...
local Button = Tab:CreateButton({
	Name = "Hoàn nguyên dữ liệu",
	Description = "Quay Xong Và Tham Gia Lại Và Đừng Sử Dụng Quá Nhiều Lượt Quay",
    	Callback = function()
...
Tab:CreateSection("Giết Ngay Lập Tức")
...
Tab:CreateSection("Chuyển Đổi")

Tab:CreateToggle({
    Name = "Chuyển Đổi đến Gần Mob",
...
local tweenPosition = "Trên Cùng" 
Tab:CreateDropdown({
    Name = "Vị trí Chuyển Đổi",
    Options = {"Trên Cùng", "Dưới", "Phía sau"},
...
Tab:CreateSection("Trùm Tự Động")
...
local Quests = Window:CreateTab({
	Name = "Nhiệm vụ hàng ngày tự động (Bật Insta Kill)",
	Icon = "looks",
	ImageSource = "Material",
...
local Toggle = Misc:CreateToggle({
    Name = "Kích hoạt mục ESP",
    Description = "Bật tính năng ESP của mục để hiển thị khoảng cách và tên mục tỉêu.",
...
Tab:CreateSection("Tự Động Đóng Băng")
...
local Toggle = Tab:CreateToggle({
    Name = "Bật Tự Động Đóng Băng",
    Description = "Bật hoặc Tắt tính năng Tự Động Đóng Băng cho các sinh vật giống người ở gần.",
    ...
    Callback = function(Value)
        ...
        if Value then
            print("Tự Động Đóng Băng Được Bật")
            ...
            print("Tự Động Đóng Băng Đã Tắt")
        end
    end
})
...
local Slider = Tab:CreateSlider({
    Name = "Chọn phạm vi để đóng băng",
    ...
    Callback = function(Value)
        ...
        saveConfig()
        print("Phạm vi được chọn để tự động đóng băng:", autofreezeRange)
    end
})
...
local function TweenAndFireProximityPrompt(character, targetModel, speed, offset)
    ...
    if rootPart and proximityPrompt then
        ...
        print("Lời nhắc 'Thu thập' được kích hoạt cho:", targetModel.Name)
    else
        print("Không tìm thấy phần 'Thu thập' Proximity Prompt hoặc 'Root' cho:", targetModel.Name)
    end
end
...
Misc:CreateToggle({
    Name = "Tự động thu thập chiến lợi phẩm (Không bật nhiệm vụ tự động))",
...
Misc:CreateSection("Slots miễn phí bẩm sinh, Bỏ qua vòng quay")
...
Misc:CreateSection("Người trao kỹ năng (Không phải vĩnh viễn)")


local modeSelected = "Bẩm Sinh"
local DropDown = Misc:CreateDropdown({
    Name = "Chọn chế độ",
    Options = {"Bẩm Sinh", "Kỹ năng"},
    CurrentOption = {"Bẩm Sinh"},
    Callback = function(value)
        modeSelected = value
    end
})
...
local Input = Misc:CreateInput({
    Name = "Nhập Kỹ năng/Kỹ năng bẩm sinh",
    CurrentValue = "",
    TextDisappear = false,  
    Callback = function(value)
...
Misc:CreateSection("Người trao vũ khí")
...
local Input = Misc:CreateInput({
    ...
    PlaceholderText = "Nhập tên vũ khí ở đây",
    CurrentValue = "",
    Numeric = false,
...
local Button = Misc:CreateButton({
    Name = "Tặng vũ khí (Không vĩnh viễn)",
    Callback = function()
        if not toolName or toolName == "" then
            print("Error: Vui lòng nhập tên công cụ.")
            return
        end
...
local Input = Tab:CreateInput({
	Name = "Di chuyển Redeemer (Perm cần tiền và thành thạo)",
	Description = nil,
	PlaceholderText = "Đầu vào giữ chỗ",
...
local inputURL = Settings:CreateInput({
    Name = "Nhập URL Discord Webhook",
    PlaceholderText = "Dán URL Discord Webhook của bạn vào đây",
    CurrentValue = "",
    Numeric = false,
    Callback = function(Text)
        url = Text
...
function SendMessageEMBED(url, embed)
    ...
    local data = {
        ...
        ["embeds"] = {
            {
                ["title"] = embed.title,
                ["url"] = "https://discord.gg/thanhtaihy11",
    ...
    if proximityPrompt then
        proximityPrompt.Triggered:Connect(function(player)
            ...
            local embed = {
                ...
                ["footer"] = {
                    ["text"] = "https://discord.gg/thanhtaihy11"
                }
            }
